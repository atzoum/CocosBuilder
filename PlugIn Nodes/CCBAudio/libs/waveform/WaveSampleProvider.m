...
@synthesize status, statusMessage, binSize, minute, sec, delegate,audioURL,trackDuration;
...
//    clientFormat.mReserved           = 0;
	...
	lengthInSec = allSec;
    trackDuration = packetReads / 44100.0f;
	minute = allSec / 60;
...
- (OSStatus) readConsecutive:(SInt64)numFrames intoArray:(float**)audio
{
    ...
    if (!data) {
		...
		UInt32 numPackets = (UInt32)numFrames; // Frames to read
...
{
	int rangeLength = (int)normalizedData.count / pixelWide + 1;
...
static WaveSampleCache *_cache;

@implementation WaveSampleCache

+(id) sharedCache
{
    if (!_cache) {
        _cache = [[WaveSampleCache alloc] init];
    }
    return _cache;
}
...
{
    if ( (self = [super init])) {
        cacheDic = [[NSMutableDictionary dictionary] retain];
    }
    return self;
}

-(WaveSampleProvider*) providerForURL:(NSURL*) url
{
    WaveSampleProvider *result;
    result = [cacheDic objectForKey:[url path]];
    if (!result) {
        result = [[WaveSampleProvider alloc] initWithURL:url];
        [result createSampleData];
        [cacheDic setValue:result forKey:[url path]];
        [result release];
    }
    return result;
}
